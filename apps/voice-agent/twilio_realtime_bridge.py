...
def session_update(call_sid: str, caller: str, called: str, caller_context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    instructions = (
        ...
        + "\nIf the caller asks to extend the stay or add nights, validate the reservation, identify the property and current checkout, ask for the desired new checkout date if missing, and check Guesty availability for the extra nights before answering. Tell the caller clearly that availability does not confirm the extension; a human Glam Homes advisor must review and approve it. Send twilio_send_human_handoff_sms with reservation code, requested dates, and the availability result, then offer to connect the caller by phone."
